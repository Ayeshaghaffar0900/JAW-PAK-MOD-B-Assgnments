...
let mobiles = {
    ...
    realme: {
      realmiNote50: {
        brand: "Realme",
        model: " realmiNote50", // Replace with specific model name
        display: {
          size: "6.5 inches",
      ...
      realmiC67: {
        brand: "Realme",
        model: "realmiC67", // Replace with specific model name
        display: {
          size: "6.7 inches",
      ...
      realmiC53: {
        brand: "Realme",
        model: "realmiC53", // Replace with specific model name
        display: {
          size: "7. 1inches",
      ...
      realmi9: {
        brand: "Realme",
        model: "realmi9", // Replace with specific model name
        display: {
          size: "7.2 inches",
      ...
      realmi9i: {
        brand: "Realme",
        model: " realmi9i ", // Replace with specific model name
        display: {
          size: "6.9 inches",
...
let model = document.getElementById("dropdown-model")
let detailsCard = document.getElementById('details-card');

function selectBrand(){
for(key in mobiles){
    brandOption = document.createElement('option')
    textBrand = document.createTextNode(key)
    brandOption.appendChild(textBrand) 
    brand.appendChild(brandOption);
    brand[0].setAttribute("disabled",false)
...
function populateModels() {
    ...
    model.innerHTML = '<option disabled value="">--Select Model--</option>'; // Clear previous models
    if (model.value !== -1) {
      for (key in mobiles[selectedBrand]) {
        modelOption = document.createElement('option')
        textModel = document.createTextNode(key)
        modelOption.appendChild(textModel) 
...
function generateDetailsHTML(details) {
    ...
    for (const key in details) {
      ...
      if (typeof value === "object" && !Array.isArray(value)) {
        html += `
          <div class="spec-card text-center p-1">
            <h6 class="spec-key">${key.toUpperCase()}:</h6>
            ${generateDetailsHTML(value)}
          </div>`;
      } else if (Array.isArray(value)) {
        html += `
          <div class="spec-card text-center  p-1">
            <h6 class="spec-key">${key.toUpperCase()}:</h6>
            <p class="spec-value">${value.join(", ")}</p>
          </div>`;
      } else {
        html += `
          <div class="spec-card text-center p-1">
  ...
  function showDetails() {
    ...
    if (selectedBrand && selectedModel && mobiles[selectedBrand][selectedModel]) {
      const details = mobiles[selectedBrand][selectedModel];
      const cardContent = generateDetailsHTML(details);
      let modelShow = details.model
      if(modelShow === undefined){
        modelShow = details.modelName
        if(modelShow === undefined){
      ...
      detailsCard.innerHTML = `
        <div class="card">
          <div class="card-body ">
            <h1 class="card-title text-center m-4">${modelShow.toUpperCase()}</h1>
            <h3 class="card-subtitle text-danger m-4    text-center">${details.brand.toUpperCase()}</h3>
            ${cardContent}
          </div>
        </div>
      `;
    } else {
      detailsCard.innerHTML = `<div class="alert alert-secondary">Please select a valid brand and model.</div>`;
    }
  }
